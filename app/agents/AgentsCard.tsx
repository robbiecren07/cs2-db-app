      ...
      <Card>
        <CardContent className="flex flex-col w-full h-full justify-between p-4">
          ...
          <h3 className="font-medium transition-colors group-hover:text-white">
            {item.shortName} <span className="block text-sm text-accent-foreground  font-normal">{item.subName}</span>
          </h3>
          ...
          {item.collectionName && (
            <span className="relative text-xs text-accent-foreground font-light">{item.collectionName}</span>
          )}
