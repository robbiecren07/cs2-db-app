...
import type { GraffitiWithRarity, RarityId } from '@/types/custom'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const data = await db
    .select({ id: schema.graffiti.id, name: schema.graffiti.name, image: schema.graffiti.image })
    .from(schema.graffiti)
    .where(eq(schema.graffiti.slug, slug))
    .limit(1)
  ...
  return {
    ...
    description: `Discover the ${data[0].name} graffiti in Counter-Strike 2. Check current Steam market prices and explore graffiti details.`,
    alternates: { canonical: `/graffiti/${slug}` },
    openGraph: {
...
async function getData(slug: string): Promise<GraffitiWithRarity | null> {
  const data = await db
    .select({
...
export default async function GraffitiItemPage({ params }: Props) {
  const { slug } = await params
  ...
  return (
    <InternalContainer>
      <BreadCrumbBar active={data.name} parent="Graffiti" parentHref="/graffiti" />
