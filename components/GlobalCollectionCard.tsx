...
export default function GlobalCollectionCard({ collection }: Props) {
  return (
    <div className="w-1/3 p-2 group">
      <Link href={`/collections/${collection.slug}`} className="block w-full" target="_self">
        <div className="w-full mx-auto flex flex-col items-center gap-2">
          ...
          <h3 className="text-sm text-center transition-colors group-hover:text-purple-600">{collection.name}</h3>
        </div>
      </Link>
    </div>
  )
